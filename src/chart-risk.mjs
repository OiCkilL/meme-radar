// 保守偏好过滤器，非诈骗定罪。复用现有已收盘1分钟K线样本，不推断未见或秒级历史。
...
export function applyRiskExclusion(row, exclusions = {}, chain = row.chain) {
  ...
  return {
    ...row,
    status: 'HARD_REJECT',
    decisionReason: Array.isArray(held.reasons) ? held.reasons.join('；') : row.decisionReason,
    deep: {
      ...row.deep,
      chainPass: false,
      checks: { ...row.deep?.checks, chartRisk: false },
      failed: [...new Set([...(row.deep?.failed || []), 'chartRisk'])],
      chartRisk: { ...row.deep?.chartRisk, ...held, status: 'REJECT', pass: false }
    }
  };
}
...
export function chartRiskScreen(candles, now = Date.now()) {
  ...
  for (const raw of Array.isArray(candles) ? candles : []) {
    ...
    const row = {
      ...
      volume: number(raw?.volume ?? raw?.v)
    };
    ...
    if (time + minute > now) continue; // 不评判尚未收盘的当前K线
  ...
  const base = {
    version: CHART_RISK_VERSION,
    bars: bars.length,
    from: bars[0]?.time || 0,
    to: bars.at(-1) ? bars.at(-1).time + minute : 0,
    scope: 'observed_1m_window',
    reasons: [],
    codes: []
  };
  ...
  for (let i = 0; i < bars.length - 3; i++) {
    ...
    if (bar.volume > 0 && after.every(b => b.volume > 0) && jump >= .35 && Math.max(...closes) / Math.min(...closes) - 1 <= .15
      ...
      reasons.push('单分钟跳升≥35%后窄幅平台，当次不通过，等待复查');
      break;
    }
  }
  // 仅早前收盘价建立峰值，后续必须连续两根收盘价回撤均>=60%
  ...
  for (let i = 1; i < bars.length - 1; i++) {
    ...
    if (bars[i].volume > 0 && bars[i + 1].volume > 0 && drawdown >= .60) {
      ...
      reasons.push('已观测收盘高点后连续两根回撤≥60%，当次不通过，等待复查');
      break;
    }
  ...
  return {
    ...base,
    pass: codes.length === 0,
    status: codes.length ? 'REJECT' : 'CLEAR_IN_WINDOW',
    codes,
    reasons,
    maxConfirmedDrawdown,
    unknownFields: []
  };
}
